...
class TextDetector(BaseOCRV20):
    ...
    def __call__(self, img):
        if img.ndim == 3:
            ...
            # if batch of images passed
            lst_ori_im = []
            lst_img = []
            lst_shape_list = []

            # new variable -- with convenient name for bulk images
            input_imgs = img

            # iterate inside batch of images
            for single_img in input_imgs:
                ori_im = single_img.copy()
                data = {'image': single_img}
                data = transform(data, self.preprocess_op)
                single_img, shape_list = data
                
                if single_img is None:
                    return None, 0

                lst_ori_im.append(ori_im)
                lst_img.append(single_img)
                lst_shape_list.append(shape_list)

            # img = np.expand_dims(img, axis=0)
            # shape_list = np.expand_dims(shape_list, axis=0)

            # img = img.copy() # comment unwanted

            image_count = len(lst_img)

            imgs = np.array(lst_img)
            # shape_list_arrays = np.array(shape_list)
            ...
            post_result = self.postprocess_op(preds, lst_shape_list)
            ...
            for idx in range(image_count):
                ...
                if (self.det_algorithm == "SAST" and
                    self.det_sast_polygon) or (self.det_algorithm in ["PSE", "FCE"] and
                                            self.postprocess_op.box_type == 'poly'):
                    dt_boxes = self.filter_tag_det_res_only_clip(dt_boxes, lst_ori_im[idx].shape)
                else:
                    dt_boxes = self.filter_tag_det_res(dt_boxes, lst_ori_im[idx].shape)
                lst_dt_boxes.append(dt_boxes)

            elapse = time.time() - starttime
            return lst_dt_boxes, elapse
